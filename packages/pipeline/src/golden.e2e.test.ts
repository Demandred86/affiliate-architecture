...
describe("golden M2 pipeline", () => {
  it("runs the actual 44-row CSV twice without duplicating canonical keywords", async () => {
    ...
    try {
      ...
      expect(
        first.report.keywords.every(
          (row) =>
            row.m1_hypothesis.metric_name === "M1_HYPOTHESIS_SCORE" &&
            ...
            row.provisional_opportunity_score.score_kind ===
              "OPPORTUNITY_SCORE" &&
            row.provisional_opportunity_score.missing_inputs.includes(
              "search_volume",
            ),
        ),
      ).toBe(true);
      ...
      const json = JSON.parse(
        await readFile(first.artifacts!.jsonPath, "utf8"),
      ) as typeof first.report;
      ...
      expect(csv.every((row) => row.m1_hypothesis_source_type === "HYPOTHESIS")).toBe(
        true,
      );
      ...
      expect(
        new Set(
          (await testDb.db.select().from(keywords)).map(
            (row) => row.canonicalHash,
          ),
        ).size,
      ).toBe(44);
      expect(await testDb.db.select().from(keywordScores)).toHaveLength(10);
      expect(
        (await testDb.db.select().from(keywordMetrics)).every(
          (metric) => metric.sourceType === "HYPOTHESIS",
        ),
      ).toBe(true);
    } finally {
      await testDb.close();
