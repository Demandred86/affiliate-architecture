...
import { AgentRunner, type BudgetCaps, type DeterministicJob } from "@ase/agent-core";
...
function qualifierFacet(value: string) {
  if (value === "lightweight") {
    return { kind: "ATTRIBUTE" as const, slug: "lightweight", label: value };
  }
  if (value.includes("beginner")) {
    return { kind: "USER" as const, slug: "beginners", label: value };
  }
  if (value.includes("elderly")) {
    return { kind: "USER" as const, slug: "elderly", label: value };
  }
  if (value.includes("small hands")) {
    return { kind: "CONSTRAINT" as const, slug: "small-hands", label: value };
  }
  if (value.includes("tomato")) {
    return { kind: "USE_CASE" as const, slug: "tomatoes", label: value };
  }
  return {
...
export function analyzeKeyword(keyword: string): KeywordAnalysis {
  ...
  let patternType: KeywordAnalysis["patternType"] = "OTHER";
  ...
  return keywordAnalysisSchema.parse({
    patternType,
    intentType:
      patternType === "OTHER" ? "UNKNOWN" : "COMMERCIAL_INVESTIGATION",
    productText,
    qualifierText,
    facets: [
    ...
    problemText: null,
    environmentText:
      qualifier?.kind === "ENVIRONMENT" ? qualifierText : null,
    constraintText:
      qualifier?.kind === "CONSTRAINT" || qualifier?.kind === "ATTRIBUTE"
        ? qualifierText
        : null,
    clusterSuggestion,
...
const analysisJob: DeterministicJob<AnalysisInput, KeywordAnalysis> = {
  agentId: "keyword",
  agentVersion: "1.0.0",
  prompt: {
    ...
    version: "1",
    content:
      "Apply the versioned BEST_X_FOR_Y and BEST_ATTRIBUTE_X grammar and lexicon.",
  },
  inputSchema,
...
export async function analyzeNicheKeywords(
  db: Database,
  nicheSlug: string,
  budgetCaps: BudgetCaps,
): Promise<AnalyzeNicheResult> {
  ...
  for (const item of nicheKeywords) {
    ...
    await db
      ...
      .where(
        and(
          eq(keywords.id, item.keyword.id),
          eq(keywords.nicheId, item.niche.id),
        ),
      );
  }
