...
const job: DeterministicJob<{ keyword: string }, { normalized: string }> = {
  agentId: "keyword",
  agentVersion: "1.0.0",
  prompt: {
    name: "analyze",
...
describe("AgentRunner", () => {
  it("persists deterministic provenance and reuses an identical run", async () => {
    ...
    try {
      ...
      expect(
        costs.reduce((sum, event) => sum + Number(event.estimatedCostUsd), 0),
      ).toBe(0);
    } finally {
      await testDb.close();
    }
...
describe("three-level budget guard", () => {
  ...
  ] as const)("refuses positive spend when %s remaining is zero", (level, usage) => {
  ...
  it("allows a zero-dollar deterministic operation at the caps", () => {
    expect(
      checkBudget(
        caps,
        { runSpentUsd: 0.05, dailySpentUsd: 1, projectSpentUsd: 5 },
        0,
      ),
