...
describe("M1 CSV importer", () => {
    ...
    try {
      ...
      expect(
        metrics.filter((metric) =>
          ["M1_HYPOTHESIS_SCORE", "M1_HYPOTHESIS_SERP_LABEL"].includes(
            metric.metricName,
          ),
        ),
      ).toHaveLength(88);
      expect(metrics.every((metric) => metric.sourceType === "HYPOTHESIS")).toBe(
        true,
      );
      expect(metrics.some((metric) => metric.sourceType === "MEASURED")).toBe(
        false,
      );
  ...
  it("reports extra-column rows as rejected and the batch as partial", async () => {
    ...
    try {
      ...
      expect(rows.find((row) => row.status === "REJECTED")?.rejectReason).toBe(
        "EXPECTED_7_COLUMNS_GOT_8",
      );
    } finally {
      await testDb.close();
    }
