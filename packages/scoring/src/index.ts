...
function commercialIntent(intent: ScoringInput["intentType"]): number {
...
const scoringJob: DeterministicJob<ScoringInput, OpportunityScore> = {
  agentId: "scoring",
  ...
  prompt: {
    ...
    content:
      "Apply approved opportunity-v1 analysis-only weights. Never use M1 hypotheses or invent commercial metrics.",
  },
...
export async function scoreNicheKeywords(
  db: Database,
  nicheSlug: string,
  budgetCaps: BudgetCaps,
): Promise<ScoreNicheResult> {
  const rows = await db
