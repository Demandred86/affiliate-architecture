import type { HandSizeInput, HandSizeRecommendation, HandSizePriority } from "./types.js";
...
function inOkatsuneSmallRange(handLengthCm: number, palmWidthCm: number): boolean {
  return (
...
function slightlyLargerHands(handLengthCm: number, palmWidthCm: number): boolean {
  return (
    handLengthCm > OKATSUNE_HAND_LENGTH_MAX &&
    handLengthCm <= 20 &&
    palmWidthCm >= OKATSUNE_PALM_WIDTH_MIN &&
...
function recommendByPriority(priority: HandSizePriority): HandSizeRecommendation | null {
  switch (priority) {
 ...
 */
export function recommendPruner(input: HandSizeInput): HandSizeRecommendation {
