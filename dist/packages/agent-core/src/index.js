...
import { agentPrompts, agentRuns, costEvents, } from "@ase/database";
import { and, eq } from "drizzle-orm";
import { v7 as uuidv7 } from "uuid";
import { ZodError } from "zod";
export function budgetCapsFromConfig(config) {
    return {
...
export function checkBudget(caps, usage, estimatedCostUsd) {
...
export class BudgetGuard {
    ...
    caps;
    now;
    constructor(db, caps, now = () => new Date()) {
        this.db = db;
        this.caps = caps;
        this.now = now;
    }
    async assertCanSpend(estimatedCostUsd, runSpentUsd = 0) {
        ...
        const projectSpentUsd = events.reduce((sum, event) => sum + Number(event.cost), 0);
        ...
        const decision = checkBudget(this.caps, { runSpentUsd, dailySpentUsd, projectSpentUsd }, estimatedCostUsd);
        if (!decision.allowed) {
...
export class AgentRunner {
    ...
    async persistPrompt(job, contentHash) {
        const [existing] = await this.db
            .select()
            .from(agentPrompts)
            .where(and(eq(agentPrompts.agentId, job.agentId), eq(agentPrompts.name, job.prompt.name), eq(agentPrompts.version, job.prompt.version)))
            .limit(1);
        if (existing && existing.contentHash !== contentHash) {
            throw new Error(`Prompt ${job.agentId}/${job.prompt.name}@${job.prompt.version} is immutable`);
        }
        if (!existing) {
