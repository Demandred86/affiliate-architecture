...
import { agentRuns, costEvents, keywordAnalyses, keywordClusterMembers, keywordMetrics, keywordScores, keywords, niches, } from "@ase/database";
...
export function defaultBudgetCaps() {
...
async function buildReport(db, nicheSlug, summary) {
    ...
    const analyses = keywordIds.length === 0
        ? []
        : await db
            .select()
            .from(keywordAnalyses)
            .where(inArray(keywordAnalyses.keywordId, keywordIds));
    const scores = keywordIds.length === 0
        ? []
        : await db
            .select()
            .from(keywordScores)
            .where(and(inArray(keywordScores.keywordId, keywordIds), eq(keywordScores.scoreKind, "OPPORTUNITY_SCORE")));
    const metrics = keywordIds.length === 0
        ? []
        : await db
            .select()
            .from(keywordMetrics)
    ...
    const rows = nicheKeywords
        ...
        const hypothesis = metrics.find((item) => item.keywordId === keyword.id &&
            item.metricName === "M1_HYPOTHESIS_SCORE");
        const serpLabel = metrics.find((item) => item.keywordId === keyword.id &&
        ...
        if (hypothesis.sourceType !== "HYPOTHESIS") {
            throw new Error(`M1_HYPOTHESIS_SCORE has invalid source_type ${hypothesis.sourceType}`);
        }
        return {
    ...
    return {
        ...
        summary,
        keywords: rows,
        total_estimated_cost_usd: costs.reduce((total, event) => total + Number(event.estimatedCostUsd), 0),
        llm_calls: runs.filter((run) => run.model !== "deterministic").length,
    };
}
export function reportToCsv(report) {
    return stringify(report.keywords.map((row) => ({
        keyword: row.keyword,
...
export function reportToMarkdown(report) {
    ...
    const keywordRows = report.keywords
        .map((row) => `| ${row.keyword} | ${row.provisional_opportunity_score.score.toFixed(3)} (${row.provisional_opportunity_score.band}) | ${row.m1_hypothesis.score.toFixed(3)} (HYPOTHESIS) |`)
        .join("\n");
...
export async function runPipeline(db, input) {
    ...
    const nicheKeywordIds = (await db
        .select({ id: keywords.id })
        .from(keywords)
        .innerJoin(niches, eq(keywords.nicheId, niches.id))
        .where(eq(niches.slug, niche))).map((row) => row.id);
    const memberships = nicheKeywordIds.length === 0
        ? []
        : await db
