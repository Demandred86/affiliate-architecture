...
import { agentRuns, createTestDb, keywordAnalyses, keywordClusterMembers, keywordClusters, keywords, seed, } from "@ase/database";
...
describe("deterministic keyword analysis", () => {
    ...
    it("analyzes all gardening keywords without an LLM and groups tomato support", async () => {
        ...
        try {
            ...
            const result = await analyzeNicheKeywords(testDb.db, "problem-solving-gardening", caps);
            ...
            expect(analyses.every((analysis) => analysis.path === "DETERMINISTIC")).toBe(true);
            expect(analyses.some((analysis) => analysis.patternType === "BEST_ATTRIBUTE_X")).toBe(true);
            ...
            const members = tomatoCluster
                ? await testDb.db
                    ...
                    .innerJoin(keywords, eq(keywords.id, keywordClusterMembers.keywordId))
            ...
            expect(runs.every((run) => run.model === "deterministic" &&
                run.provider === "none" &&
                run.inputTokens === 0 &&
                run.outputTokens === 0)).toBe(true);
            const repeated = await analyzeNicheKeywords(testDb.db, "problem-solving-gardening", caps);
