...
import { canonicalizeKeyword, importBatches, importRows, keywordCanonicalHash, keywordMetrics, KeywordRepository, keywords, nicheAliases, niches, } from "@ase/database";
...
export async function importM1Csv(db, input) {
    ...
    for (const alias of aliasRows) {
        ...
        if (niche)
            nicheByName.set(alias.alias, niche);
    }
    let acceptedCount = 0;
    let rejectedCount = 0;
    ...
    for (const [index, row] of records.entries()) {
        ...
        const rejectReason = row.length !== header.length
        ...
        const canonicalHash = keywordCanonicalHash(canonicalText, niche.market, niche.language);
