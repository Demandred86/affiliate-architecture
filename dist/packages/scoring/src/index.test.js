...
import { createTestDb, keywordMetrics, keywordScores, seed, } from "@ase/database";
import { importM1Csv } from "@ase/importer";
import { analyzeNicheKeywords } from "@ase/keyword-agent";
import { eq } from "drizzle-orm";
import { describe, expect, it } from "vitest";
import { calculateOpportunityScore, scoreNicheKeywords, } from "./index.js";
const caps = {
    maxCostPerRunUsd: 1,
    dailyBudgetUsd: 1,
...
describe("opportunity-v1", () => {
    it("is deterministic and has no M1 hypothesis input", () => {
        ...
        expect(after.missingInputs).toEqual(expect.arrayContaining(["search_volume", "serp_retrieved"]));
    });
    it("stores ten provisional scores and remains independent of M1 mutation", async () => {
        ...
        try {
            ...
            await analyzeNicheKeywords(testDb.db, "problem-solving-gardening", caps);
            const result = await scoreNicheKeywords(testDb.db, "problem-solving-gardening", caps);
            expect(result).toEqual({ scoredCount: 10, cacheHits: 0 });
            const before = await testDb.db.select().from(keywordScores);
            expect(before).toHaveLength(10);
            expect(before.every((score) => score.scoreKind === "OPPORTUNITY_SCORE" &&
                score.modelId === "opportunity-v1" &&
                score.score !== null &&
                score.band.startsWith("PROVISIONAL_") &&
                score.missingInputs.includes("search_volume") &&
                score.missingInputs.includes("serp_retrieved"))).toBe(true);
            expect(before.filter((score) => score.scoreKind === "SERP_SCORE")).toHaveLength(0);
            await testDb.db
                .update(keywordMetrics)
                .set({ numericValue: "1.000", updatedAt: new Date() })
                .where(eq(keywordMetrics.metricName, "M1_HYPOTHESIS_SCORE"));
            const repeated = await scoreNicheKeywords(testDb.db, "problem-solving-gardening", caps);
            expect(repeated).toEqual({ scoredCount: 10, cacheHits: 10 });
            const after = await testDb.db.select().from(keywordScores);
            expect(after.map((score) => score.score)).toEqual(before.map((score) => score.score));
            const hypotheses = await testDb.db
                .select()
                .from(keywordMetrics)
                .where(eq(keywordMetrics.metricName, "M1_HYPOTHESIS_SCORE"));
            expect(hypotheses.every((metric) => metric.numericValue === "1.000" &&
                metric.sourceType === "HYPOTHESIS")).toBe(true);
        }
        finally {
            await testDb.close();
        }
