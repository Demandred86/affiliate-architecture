...
import { createTestDb, ImportBatchRepository, KeywordRepository, keywords, NicheRepository, nicheAliases, niches, seed, } from "./index.js";
...
describe("lean M2 database", () => {
    it("migrates only the approved table set and seeds the niche alias", async () => {
        ...
        try {
            ...
            const result = await testDb.client.query(`select table_name
         ...
         order by table_name`);
            ...
            expect(seededNiches.filter((niche) => niche.status === "ACTIVE")).toEqual([
                { slug: "problem-solving-gardening", status: "ACTIVE" },
            ]);
            expect(seededNiches.filter((niche) => niche.status === "PARKED")).toHaveLength(4);
        }
        finally {
            await testDb.close();
        }
    ...
    it("returns the same canonical keyword and enforces its unique hash", async () => {
        ...
        try {
            ...
            await expect(testDb.db.insert(keywords).values({
                ...first,
                id: uuidv7(),
                rawText: "duplicate",
                createdAt: new Date(),
