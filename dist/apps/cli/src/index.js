...
async function main() {
    ...
    if (command === "import") {
        ...
        const result = await withDatabase((client) => importM1Csv(createDatabase(client), {
            content,
            sourcePath: filePath,
            actor: "cli",
        }));
    ...
    if (command === "analyze" || command === "score") {
        ...
        if (command === "analyze") {
            const result = await withDatabase((client) => analyzeNicheKeywords(createDatabase(client), niche, caps));
            print({ command, niche, ...result });
        }
        else {
            const result = await withDatabase((client) => scoreNicheKeywords(createDatabase(client), niche, caps));
            print({ command, niche, ...result });
        }
        return;
    }
    if (command === "pipeline") {
        ...
        const content = await readFile(filePath);
        const result = await withDatabase((client) => runPipeline(createDatabase(client), {
            content,
            sourcePath: filePath,
            niche,
            outputDirectory,
        ...
        if (result.importResult.status === "PARTIAL")
            process.exitCode = 1;
        return;
    }
...
main().catch((error) => {
    process.stderr.write(`${error instanceof Error ? error.message : "Unknown CLI failure"}\n`);
    process.exitCode = 2;
});
